...
import UIKit
/// Ratingview это звездочки спешил скопировал где то в гитхабе далее перепишу свой
...
//    @IBInspectable open
    ...
    @objc public enum FloatRatingViewType: Int {
        ...
        func supportsFractions() -> Bool {
            self == .halfRatings || self == .floatRatings
        }
    }
...
//    @IBInspectable open
    ...
    private func removeImageViews() {
        ...
        for ind in 0 ..< emptyImageViews.count {
            var imageView = emptyImageViews[ind]
            imageView.removeFromSuperview()
            imageView = fullImageViews[ind]
            imageView.removeFromSuperview()
        }
    ...
    private func refresh() {
        for ind in 0 ..< fullImageViews.count {
            let imageView = fullImageViews[ind]

            if rating >= Double(ind + 1) {
                imageView.layer.mask = nil
                imageView.isHidden = false
            } else if rating > Double(ind), rating < Double(ind + 1) {
                // Set mask layer for full image
                let maskLayer = CALayer()
                maskLayer.frame = CGRect(
                    x: 0,
                    y: 0,
                    width: CGFloat(rating - Double(ind)) * imageView.frame.size.width,
    ...
    private func updateLocation(_ touch: UITouch) {
        ...
        var newRating = 0.0
        for ind in stride(from: maxRating - 1, through: 0, by: -1) {
            let imageView = emptyImageViews[ind]
            ...
            if imageView.point(inside: newLocation, with: nil), type.supportsFractions() {
                ...
                newRating = Double(ind) + decimalNum
                if type == .halfRatings {
                    newRating = Double(ind) + (decimalNum > 0.75 ? 1 : (decimalNum > 0.25 ? 0.5 : 0))
                }
            } else {
                // Whole rating
                newRating = Double(ind) + 1.0
            }
            break
        }
    ...
    override open func layoutSubviews() {
        ...
        for ind in 0 ..< maxRating {
            let imageFrame = CGRect(
                x: ind == 0 ? 0 : CGFloat(ind) * (imageXOffset + imageViewSize.width),
                y: 0,
    ...
    override open func touchesBegan(_ touchesSet: Set<UITouch>, with event: UIEvent?) {
        guard let touch = touchesSet.first else {
            return
        }
        updateLocation(touch)
    }

    override open func touchesMoved(_ touchesSet: Set<UITouch>, with event: UIEvent?) {
        guard let touch = touchesSet.first else {
            return
        }
        updateLocation(touch)
    ...
    override open func touchesEnded(_ touchesSet: Set<UITouch>, with event: UIEvent?) {
        // Update delegate
        delegate?.floatRatingView?(self, didUpdate: rating)
    }

    override open func touchesCancelled(_ touchesSet: Set<UITouch>, with event: UIEvent?) {
        // Update delegate
        delegate?.floatRatingView?(self, didUpdate: rating)
    }
}
